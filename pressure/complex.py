    ...
    def get(self):
        ...
        for sigstr in signals:
            ...
            try:
                ...
                setting = daq.get(toget)[dev]['demods'][
                    dmnum]['enable']['value'][0]
            except KeyError:
                setting = 0
...
class Scope(MultiParameter):
    ...
    def prepare_scope(self):
        ...
        starttime = params['scope_trig_reference'].get() * \
            0.01 * duration + delay
    ...
    def get(self):
        ...
        while (zi_error or timedout) and error_counter < num_retries:
            ...
            self._instrument.daq.setInt(
                '/{}/scopes/0/single'.format(self._instrument.device), 1)
            self._instrument.daq.sync()

            # There are issues reusing the scope.
...
class ZIUHFLI(Instrument):
    ...
    def __init__(self, name, device_ID, **kwargs):
        ...
        self.add_parameter('scope_samplingrate',
                           ...
                           vals=vals.Enum(
                               *list(self._samplingrate_codes.keys()))
                           )
    ...
    def _sigout_setter(self, number, mode, setting, value):
        ...
        def amp_valid():
            ...
            if autorange_val == 'ON':
                ...
                so_range = params[
                    'signal_output{}_range'.format(number + 1)].get()
        ...
        def offset_valid():
            ...
            amp_val = params[
                'signal_output{}_amplitude'.format(number + 1)].get()
        ...
        def range_valid():
            ...
            autorange_val = toget.get()
            imp50_val = params[
                'signal_output{}_imp50'.format(number + 1)].get()
        ...
        def update_range_offset_amp():
            range_val = params[
                'signal_output{}_range'.format(number + 1)].get()
            offset_val = params[
                'signal_output{}_offset'.format(number + 1)].get()
            amp_val = params[
                'signal_output{}_amplitude'.format(number + 1)].get()
            if -range_val < offset_val + amp_val > range_val:
                # The GUI would allow higher values but it would clip the
                # signal.
        ...
        def update_range():
            self.parameters[
                'signal_output{}_autorange'.format(number + 1)].get()
    ...
    def _get_sweep_time(self):
        ...
        time_est = (settlingtime + averagingtime) * \
            self.sweeper_samplecount.get()
        return time_est
        ...
        for dm in demods:
            for feat in features:
                parameter = self.parameters[
                    'demod{:d}_{}'.format(dm + 1, feat)]
                fmt = (dm + 1, parameter.label,
                       parameter.get(), parameter.unit)
